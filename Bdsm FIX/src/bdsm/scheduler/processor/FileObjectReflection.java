		...
		try {
			...
			try {
				...
				if (success && auth) {
					...
					this.localSession = baseProcessor.getLocalSession();
					
					getLogger().info("Finish class" + javaClass + " 2nd execution. Result = " + success);
					  try {
                        updateFixLog(inboxId, StatusDefinition.DONE, idScheduler);
                    } catch (Exception e) {
                        getLogger().info("Checking 2nd Test" + javaClass + "Checking 2ndExecute");
                    }
					if (success) {
                            getLogger().info("Checking_REG_FixQXtract" + javaClass + "Checking_Reg_FixQXtract");
						...
						if ((fixQXtract = (FixQXtract) context.get(MapKey.fixQXtract)) != null) {
                            ...
                            getLogger().info("Checking_Register_FixQXtract_DONE" +javaClass + "Checking_Reg_FixQXtract_DONE");
                }
				...
				try {
                    ...
                    getLogger().info("CHECKING_SKIP_PROCESS" +javaClass + "CHECKING_SKIP_PROCESS");
                }
			}
		} catch (Exception ex) {
