...
import bdsm.scheduler.dao.TmpMaintenanceCim11SpouseDao;
...
import bdsm.scheduler.model.TmpMaintenanceCim11SpousePK;
 ...
 * @author v00020800
 */
public class MaintenanceCim11SpouseWorker extends BaseProcessor {
    ...
    public MaintenanceCim11SpouseWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_cim11Spouse.properties";
        ...
        TmpMaintenanceCim11SpouseDao cim11SpouseDao = new TmpMaintenanceCim11SpouseDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            this.readExcel(param5, configFile, cim11SpouseDao); //Must Change
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            cim11SpouseDao.runValidate(batchNo);
            ...
            cim11SpouseDao.runProcess(batchNo);
            ...
            cim11SpouseDao.runReject(batchNo);
            fixQXtract = new FixQXtract();
    ...
    private void readExcel(String param5, String configFile, TmpMaintenanceCim11SpouseDao cim11SpouseDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, cim11SpouseDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, cim11SpouseDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMaintenanceCim11SpouseDao cim11SpouseDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMaintenanceCim11Spouse cim11Spouse;
        TmpMaintenanceCim11SpousePK cim11SpousePK;
        String[] propertyNames = {"compositeId.cif", "spouseParentName", "sSpouseBD","spouseParentDtls", "spouseParentIc", "spouseParentOccp", "address1", "address2", 
            "address3", "phoneNo", "spouseParentDesgn"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            cim11SpousePK = new TmpMaintenanceCim11SpousePK();
            cim11SpousePK.setBatchiId(context.get(MapKey.batchNo).toString());
            cim11SpousePK.setRecordId(recordId);
            cim11Spouse = new TmpMaintenanceCim11Spouse();
            cim11Spouse.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            cim11Spouse.setCompositeId(cim11SpousePK);
            
            try {
                if (xr.nextRow(cim11Spouse, propertyNames)) {
                    if (cim11Spouse.getsSpouseBD() != null) {
                        try {
                            cim11Spouse.setSpouseParentBirthDate(sdf.parse(cim11Spouse.getsSpouseBD()));
                        } catch (Exception e) {
                            cim11Spouse.setFlagStatus("R");
                            cim11Spouse.setStatusReason("Invalid Format Spouse/Parent Bisrth Date, must be DD/MM/YYYY");
                        }
                    }
                    cim11SpouseDao.insert(cim11Spouse);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                cim11Spouse.setStatusReason(ex.toString());
                cim11Spouse.setFlagStatus(StatusDefinition.REJECTED);
                cim11SpouseDao.insert(cim11Spouse);
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpMaintenanceCim11SpouseDao cim11SpouseDao) {
