...
import bdsm.service.SknNgDebitService;
...
public class SknNgOutwardDebitAction extends BaseHostAction implements org.apache.struts2.interceptor.ApplicationAware {
	...
	public String doUpdate() {
		...
		SknNgInOutDebitDetail detail = (SknNgInOutDebitDetail) ((Object[]) mapDetail.get(new SknNgPK(this.batchNo, this.recordId)))[1];
		...
		while (it.hasNext()) {
			...
			try {
				...
				this.getLogger().warn("No property of Object \"" + SknNgInOutDebitDetail.class + "\" with name: \"" + key + "\"");
			}
		}
    ...
    public String listFilename() {
		...
		this.listAuditBatch = batchDAO.listByBatchNoTypeAndStatus(
				this.branchNo + "-%",
				Integer.valueOf(SknNgDebitService.CODE_DKE), 
				status, 
				new java.util.Date()
			);
	...
	public String listHeader() {
		...
		this.listHeader = headerDAO.listDebitByBatchNoAndStatus(this.batchNo, status);
	...
	public String listDetail() {
		...
		if (this.batchNo.equals(this.sessionMap.get("batchNo")) == false) {
			...
			SknNgWSAuditTrailDetailDAO detailDAO = new SknNgWSAuditTrailDetailDAO(this.getHSession());
			this.listDetail = detailDAO.listDebitByBatchNoAndStatus(this.batchNo, Integer.valueOf(this.recordId), status); 
			/* [0] = SknNgWSAuditTrailDetail; [1] = SknNgInOutDebitDetail; [2] = SPK Error Description */
			...
			for (Object o : this.listDetail) {
				...
				SknNgInOutDebitDetail dOri = (SknNgInOutDebitDetail) ((Object[]) o)[1];
	...
	public String sendToSPKWebService() {
		SknNgInOutDebitHeaderDAO headerDAO = new SknNgInOutDebitHeaderDAO(this.getHSession());
		SknNgInOutDebitDetailDAO detailDAO = new SknNgInOutDebitDetailDAO(this.getHSession());
		SknNgInOutDebitFooterDAO footerDAO = new SknNgInOutDebitFooterDAO(this.getHSession());
		...
		try {
			...
			SknNgInOutDebitHeader modelHeader = headerDAO.get(new SknNgPK(this.batchNo, this.recordId));
			...
			Iterator<SknNgPK> it = mapDetail.keySet().iterator();
			while (it.hasNext()) {
				SknNgInOutDebitDetail modelDetail, modelDetailPersist;
				
				SknNgPK pk = it.next();
				modelDetail = (SknNgInOutDebitDetail) ((Object[]) mapDetail.get(pk))[1];
				listRecordId.add(modelDetail.getCompositeId().getRecordId());
				
				modelDetailPersist = (SknNgInOutDebitDetail) detailDAO.merge(modelDetail);
			...
			SknNgInOutDebitFooter modelFooter = footerDAO.get(this.batchNo, this.recordId);
