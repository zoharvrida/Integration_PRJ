...
import bdsm.scheduler.dao.TmpMpCompDao;
 ...
 * @author v00020841
 */
public class MpCompWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process mp_comp product mitra pasti has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process mp_comp product mitra pasti has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";

    public MpCompWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        ...
        TmpMpCompDao mpCompDao = new TmpMpCompDao(session);
...
//                    dateFormatter.parse(dateInFilename);
            ...
            this.readExcel(param5, configFile, mpCompDao);
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            mpCompDao.runValidate(batchNo);
    ...
    private void readExcel(String param5, String configFile, TmpMpCompDao mpCompDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, mpCompDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, mpCompDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMpCompDao mpCompDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMpComp tmpMpComp;
        TmpMpPK tmpMPPK;
        String[] propertyNames = {"codComp","namComp","typComp","typData","txtNarrative", "cmd"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            tmpMpComp = new TmpMpComp();
            tmpMPPK = new TmpMpPK();
            tmpMPPK.setBatchNo(context.get(MapKey.batchNo).toString());
            tmpMPPK.setRecordId(recordId);
            tmpMpComp.setCompositeId(tmpMPPK);
            tmpMpComp.setIdCreatedBy("BDSM");
            tmpMpComp.setIdCreatedSpv("BDSM");
            tmpMpComp.setDtmCreated(SchedulerUtil.getTime());
            tmpMpComp.setDtmUpdated(SchedulerUtil.getTime());
            tmpMpComp.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(tmpMpComp, propertyNames)) {
                    mpCompDao.insert(tmpMpComp);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                tmpMpComp.setFlagStatus(StatusDefinition.REJECTED);
                tmpMpComp.setStatusReason(ex.toString());
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpMpCompDao mpCompDao) {
