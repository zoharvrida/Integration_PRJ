 ...
 * @author bdsm
 */
public class ChangeStatusWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process Change Status has been Processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process Change Status Update has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public ChangeStatusWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_changestatus.properties";

        ChangeStatusTmpDao changeStatusTmpDao = new ChangeStatusTmpDao(session);
        ...
        ChangeStatusTmp changeStatusTmp = null;
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            String outFileName = FileUtil.getDateTimeFormatedFileName(FilenameUtils.getBaseName(context.get(MapKey.param1).toString().replace("CHANGESTATUS ", "")) + "{hhmmss}.xls");
            getLogger().info("Out File Name : " + outFileName);
            String param5 = context.get(MapKey.param5).toString();
            getLogger().info("Param 5 : " + param5);

            readExcel(param5, configFile, changeStatusTmp, changeStatusTmpDao);
            changeStatusTmpDao.get(BatchID);
            getLogger().info("Remove Bug");
            changeStatusTmpDao.runValidate(BatchID);
            getLogger().info("Run validate Change Status");
            ...
            if (!sourceProcess.equalsIgnoreCase("sftp")) {
                ...
                getLogger().info("ChangeStatusWorker Register FixQXtract Done");
            }

            return true;
        } else if (status.equals(StatusDefinition.AUTHORIZED)) {
            getLogger().info("Status : AUTHORIZED");
            changeStatusTmpDao.runUpdate(BatchID);
            getLogger().info("Run update to Change Status User Done");
            String out2FileName = FileUtil.getDateTimeFormatedFileName(FilenameUtils.getBaseName(context.get(MapKey.param1).toString().replace("RE: CHANGESTATUS ", "")) + "{hhmmss}.xls");
            getLogger().info("ChangeStatusWorker Register FixQXtract");
            ...
            getLogger().info("ChangeStatusWorker Register FixQXtract");
            changeStatusTmpDao.runReject(BatchID);
    ...
    private void readExcel(String param5, String configFile, ChangeStatusTmp changeStatusTmp, ChangeStatusTmpDao changeStatusTmpDao) throws FIXException, FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        loadConfig(configFile);
        
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, changeStatusTmp, changeStatusTmpDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, changeStatusTmp, changeStatusTmpDao);
        }
    }

    private void readXLS(String param5, ChangeStatusTmp changeStatusTmp, ChangeStatusTmpDao changeStatusTmpDao) throws FIXException, FileNotFoundException, IOException {
        BufferedInputStream file = new BufferedInputStream(new FileInputStream(new File(param5)));
        getLogger().info("Start Reading Change Status File.");
        ...
        try {            
            ...
            for (int i = rowDataStartOn - headerRow; i < rowCnt - headerRow; i++) {
                ...
                int codAcctStatCurr = getIntegerCellVal(row.getCell(j + 2));
                ...
                changeStatusTmp = new ChangeStatusTmp();
                changeStatusTmp.setCodAcctNo(codAcctNo);
                changeStatusTmp.setCodAcctStatCurr(codAcctStatCurr);
                changeStatusTmp.setNoBatch(noBatch);
                changeStatusTmpDao.insert(changeStatusTmp);
            }
        } catch (Exception e) {
    ...
    private void readXLSX(String param5, ChangeStatusTmp changeStatusTmp, ChangeStatusTmpDao changeStatusTmpDao) throws FIXException, FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        ...
        processSheet(styles, strings, stream, changeStatusTmp, changeStatusTmpDao);
        stream.close();
    }
     ...
     * @param changeStatusTmp
     * @param changeStatusTmpDao
     * @throws FIXException
     * @throws IOException
     * @throws ParserConfigurationException
     * @throws SAXException
     */
    public void processSheet(StylesTable styles, ReadOnlySharedStringsTable strings, InputStream sheetInputStream, ChangeStatusTmp changeStatusTmp, ChangeStatusTmpDao changeStatusTmpDao) throws FIXException, IOException, ParserConfigurationException, SAXException {
        int minColumns = -1;
        InputSource sheetSource = new InputSource(sheetInputStream);
        SAXParserFactory saxFactory = SAXParserFactory.newInstance();
        SAXParser saxParser = saxFactory.newSAXParser();
        XMLReader sheetParser = saxParser.getXMLReader();
        ContentHandler handler = new MyXSSFSheetHandler(styles, strings, minColumns, System.out, changeStatusTmp, changeStatusTmpDao);
    ...
    private void loadConfig(String confFile) throws FileNotFoundException, IOException {
        ...
        InputStream in = ChangeStatusWorker.class.getClassLoader().getResourceAsStream(confFile);
        properties.load(in);        
    ...
    class MyXSSFSheetHandler extends DefaultHandler {
        ...
        private ChangeStatusTmp ufcr;
        private ChangeStatusTmpDao ufcrDao;

        public MyXSSFSheetHandler(StylesTable styles, ReadOnlySharedStringsTable strings, int cols, PrintStream target, ChangeStatusTmp changeStatusTmp, ChangeStatusTmpDao changeStatusTmpDao) {
            ...
            this.ufcr = changeStatusTmp;
            this.ufcrDao = changeStatusTmpDao;
            this.ufcr = new ChangeStatusTmp();
        }
    }       
