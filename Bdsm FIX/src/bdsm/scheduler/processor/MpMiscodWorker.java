...
import bdsm.scheduler.dao.TmpMpMiscodDao;
...
import bdsm.scheduler.model.TmpMpMiscod;
 ...
 * @author v00020841
 */
public class MpMiscodWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process mis_cod product mitra pasti has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process mis_cod product mitra pasti has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public MpMiscodWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        ...
        TmpMpMiscodDao mpMiscodDao = new TmpMpMiscodDao(session);
...
//                    dateFormatter.parse(dateInFilename);
            ...
            this.readExcel(param5, configFile, mpMiscodDao);
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            mpMiscodDao.runValidate(batchNo);
            ...
            mpMiscodDao.runProcess(batchNo);
    ...
    private void readExcel(String param5, String configFile, TmpMpMiscodDao mpMiscodDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, mpMiscodDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, mpMiscodDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMpMiscodDao mpMiscodDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMpMiscod tmpMpMiscod;
        TmpMpPK tmpMPPK;
        String[] propertyNames = {"codProd","codMis","prodMisName","cmd"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            tmpMpMiscod = new TmpMpMiscod();
            tmpMPPK = new TmpMpPK();
            tmpMPPK.setBatchNo(context.get(MapKey.batchNo).toString());
            tmpMPPK.setRecordId(recordId);
            tmpMpMiscod.setCompositeId(tmpMPPK);
            tmpMpMiscod.setIdCreatedBy("BDSM");
            tmpMpMiscod.setIdCreatedSpv("BDSM");
            tmpMpMiscod.setDtmCreated(SchedulerUtil.getTime());
            tmpMpMiscod.setDtmUpdated(SchedulerUtil.getTime());
            tmpMpMiscod.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(tmpMpMiscod, propertyNames)) {
                    mpMiscodDao.insert(tmpMpMiscod);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                tmpMpMiscod.setFlagStatus(StatusDefinition.REJECTED);
                tmpMpMiscod.setStatusReason(ex.toString());
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpMpMiscodDao mpMiscodDao) {
