...
public class GeneralAutoReplyWriter extends BaseProcessor {
    ...
    public boolean xlsWriter(String filename) {
		...
		for (int i = 0; i < arDetails.size(); i++) {
			ARParamDetails ard = arDetails.get(i);
			headerNames[i] = ard.getFieldName();
			if (("String".equalsIgnoreCase(ard.getFieldType())) && (ard.getFieldFormat() != null))
				hedField
					.append("CAST(")
					.append(ard.getFieldName())
					.append(" AS ")
					.append(ard.getFieldFormat())
					.append((ard.getMaxLength()!=null)? "(" + ard.getMaxLength()  +")": "")
					.append(") AS ")
					.append(ard.getFieldName());
			else
				hedField.append(ard.getFieldName());
