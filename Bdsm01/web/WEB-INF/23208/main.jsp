...
<s:if test="%{#request.requestMethod.equals('POST')}">
	<s:url var="ajaxUpdateTitle" action="23208_title" />
	<s:url var="ajaxUpdateButtons" action="23208_buttons" />
	...
	<s:form id="frmDlgAuth" action="dlg">
		...
		<s:hidden name="idMenu" value="23208" />
	</s:form>
	<sj:a id="aDlgAuth" formIds="frmDlgAuth" targets="ph-dlg" cssClass="ui-helper-hidden" />
	
	<s:form id="frmMain" name="frmMain" action="23208" method="post" validate="true">
		<s:select 
			name="batchNo"
			list="mapFilename"
			emptyOption="true"
	...
	<s:url var="urlTab" action="23208Tab_none" />
	
	<sj:tabbedpanel id="tabbedPanelHeader" disabled="true" cssStyle="max-width:950px">
		<sj:tab id="tabHeader" href="%{urlTab}" label="SKNNG Retur Inward Debit Header" />
	</sj:tabbedpanel>
	...
	<fieldset id="fsData" class="ui-widget-content ui-corner-all ui-helper-hidden">
		...
		<s:form id="frmData" name="frmData" action="23208_execute" method="post" theme="css_xhtml">
			<table>
				<tbody>
					<tr>
						<td>
					...
					<tr>
						...
						<td>&nbsp;</td>
						<td>
							<s:label id="lblSenderSectorCode" key="label.sender.sector.code" />
						</td>
						<td>
							<s:textfield
								name="senderSectorCode"
								readonly="true"
								cssClass="ui-widget ui-widget-content"
							/>
						</td>
			...
			<sj:tabbedpanel id="tabbedPanelDetail" disabled="true" cssStyle="max-width:900px">
				<sj:tab id="tabDetail" href="%{urlTab}" label="SKNNG Retur Inward Debit Detail" />
			</sj:tabbedpanel>
	...
	<script type="text/javascript">
		$(function() {
			...
			$("#btnFind").click(function() {
				...
				$("#tabbedPanelHeader").tabs("url", 0, "23208Tab_execute.action");
