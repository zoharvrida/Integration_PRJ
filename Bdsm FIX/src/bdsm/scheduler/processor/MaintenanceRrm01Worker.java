...
import bdsm.scheduler.dao.TmpMaintenanceRrm01Dao;
...
import bdsm.scheduler.model.TmpMaintenanceRrm01;
import bdsm.scheduler.model.TmpMaintenanceRrm01PK;
 ...
 * @author v00020841
 */
public class MaintenanceRrm01Worker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process maintenance cif rrm01  has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process to maintenance cif rrm01 has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public MaintenanceRrm01Worker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_rrm01.properties";
        ...
        TmpMaintenanceRrm01Dao rrm01Dao = new TmpMaintenanceRrm01Dao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            this.readExcel(param5, configFile, rrm01Dao); //Must Change
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            rrm01Dao.runValidate(batchNo);
    ...
    private void readExcel(String param5, String configFile, TmpMaintenanceRrm01Dao rrm01Dao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, rrm01Dao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, rrm01Dao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMaintenanceRrm01Dao rrm01Dao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMaintenanceRrm01 rrm01;
        TmpMaintenanceRrm01PK rrm01PK;
        String[] propertyNames = {"compositeId.codId","cmd","namAlias","namMother","golPemilik",
                                  "statusPemilik","branchLocation","sandiData","relBank","relConnBank"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            rrm01PK = new TmpMaintenanceRrm01PK();
            rrm01PK.setBatchiId(context.get(MapKey.batchNo).toString());
            rrm01PK.setRecordId(recordId);
            rrm01 = new TmpMaintenanceRrm01();
            rrm01.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            rrm01.setCompositeId(rrm01PK);
            try {
                if (xr.nextRow(rrm01, propertyNames)) {
                    rrm01Dao.insert(rrm01);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                rrm01.setStatusReason(ex.toString());
                rrm01.setFlagStatus(StatusDefinition.REJECTED);
                rrm01Dao.insert(rrm01);
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpMaintenanceRrm01Dao rrm01Dao) {
