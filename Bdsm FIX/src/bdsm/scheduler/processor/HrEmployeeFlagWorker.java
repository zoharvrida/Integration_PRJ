...
import bdsm.scheduler.dao.HrEmployeeFlagDao;
...
import bdsm.scheduler.model.TmpHrEmployeeFlagPK;
 ...
 * @author v00020841
 */
public class HrEmployeeFlagWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process Update Employee Flag Account has been Processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process to Update Employee Flag Account has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public HrEmployeeFlagWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_hremployeeflag.properties";
        ...
        HrEmployeeFlagDao hrEmployeeFlagDao = new HrEmployeeFlagDao(session);
...
//                    dateFormatter.parse(dateInFilename);
            ...
            this.readExcel(param5, configFile, hrEmployeeFlagDao);
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            hrEmployeeFlagDao.runUpload(batchNo);
            ...
            hrEmployeeFlagDao.runUpdate(batchNo);
            ...
            hrEmployeeFlagDao.runReject(batchNo);
            fixQXtract = new FixQXtract();
    ...
    private void readExcel(String param5, String configFile, HrEmployeeFlagDao hrEmployeeFlagDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, hrEmployeeFlagDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, hrEmployeeFlagDao);
        }
    }

    private void readXLS(String param5, String configFile, HrEmployeeFlagDao hrEmployeeFlagDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpHrEmployeeFlag tmpHrEmployeeFlag;
        TmpHrEmployeeFlagPK tmpHrEmployeeFlagPK;
        String[] propertyNames = {"compositeId.codAcctNo", "flgEmpAcct"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            tmpHrEmployeeFlag = new TmpHrEmployeeFlag();
            tmpHrEmployeeFlagPK = new TmpHrEmployeeFlagPK();
            tmpHrEmployeeFlagPK.setBatchNo(context.get(MapKey.batchNo).toString());
            tmpHrEmployeeFlagPK.setRecordId(recordId);
            tmpHrEmployeeFlag.setCompositeId(tmpHrEmployeeFlagPK);
            tmpHrEmployeeFlag.setCodLastMntMakerid("BDSM");
            tmpHrEmployeeFlag.setCodLastMntChkrid("BDSM");
            tmpHrEmployeeFlag.setDatLastMnt(SchedulerUtil.getTime());
            tmpHrEmployeeFlag.setFlgStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(tmpHrEmployeeFlag, propertyNames)) {
                    hrEmployeeFlagDao.insert(tmpHrEmployeeFlag);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                tmpHrEmployeeFlag.setFlgStatus(StatusDefinition.REJECTED);
                tmpHrEmployeeFlag.setStatusReason(ex.toString());
            }
        }
    }

    private void readXLSX(String param5, String configFile, HrEmployeeFlagDao hrEmployeeFlagDao) {
