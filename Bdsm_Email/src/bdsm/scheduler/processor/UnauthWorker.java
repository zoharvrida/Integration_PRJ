...
public class UnauthWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process Unauthorize has been Processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process Unauthorize Update has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";

    public UnauthWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_unauth.properties";

        UnauthTmpDao unauthTmpDao = new UnauthTmpDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            String outFileName = FileUtil.getDateTimeFormatedFileName(FilenameUtils.getBaseName(context.get(MapKey.param1).toString().replace("UNAUTH ", "")) + "{hhmmss}.xls");
            getLogger().info("Out File Name : " + outFileName);
            String param5 = context.get(MapKey.param5).toString();
            getLogger().info("Param 5 : " + param5);

            readExcel(param5, configFile, unauthTmp, unauthTmpDao);
            unauthTmpDao.get(BatchID);
            getLogger().info("Remove Bug");
            unauthTmpDao.runValidate(BatchID);
            getLogger().info("Run validate Unauth");
            ...
            if (!sourceProcess.equalsIgnoreCase("sftp")) {
                ...
                getLogger().info("UnauthWorker Register FixQXtract Done");
            }

            return true;
        } else if (status.equals(StatusDefinition.AUTHORIZED)) {
            getLogger().info("Status : AUTHORIZED");
            unauthTmpDao.runUpdate(BatchID);
            getLogger().info("Run update to Unauth User Done");
            String out2FileName = FileUtil.getDateTimeFormatedFileName(FilenameUtils.getBaseName(context.get(MapKey.param1).toString().replace("RE: UNAUTH ", "")) + "{hhmmss}.xls");
            getLogger().info("UnauthWorker Register FixQXtract");
            ...
            getLogger().info("UnauthWorker Register FixQXtract");
            unauthTmpDao.runReject(BatchID);
    ...
    private void readExcel(String param5, String configFile, UnauthTmp unauthTmp, UnauthTmpDao unauthTmpDao) throws FIXException, FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        loadConfig(configFile);
        
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, unauthTmp, unauthTmpDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, unauthTmp, unauthTmpDao);
        }
    }

    private void readXLS(String param5, UnauthTmp unauthTmp, UnauthTmpDao unauthTmpDao) throws FIXException, FileNotFoundException, IOException {
        BufferedInputStream file = new BufferedInputStream(new FileInputStream(new File(param5)));
        getLogger().info("Start Reading Unauth File.");
        ...
        try {
            ...
            for (int i = rowDataStartOn - headerRow; i < rowCnt - headerRow; i++) {
                ...
                String idPatch = getStringCellVal(row.getCell(j + 1));
                String codID = getStringCellVal(row.getCell(j + 2));
                String noBatch = context.get(MapKey.batchNo).toString();

                getLogger().info("ID Patch : " + idPatch);
                getLogger().info("COD ID : " + codID);

                if (idPatch == null || "".equals(idPatch)) {
                    continue;
                }
                
                unauthTmp = new UnauthTmp();
                unauthTmp.setIdPatch(idPatch);
                unauthTmp.setCodID(codID);
                unauthTmp.setNoBatch(noBatch);                 
                unauthTmpDao.insert(unauthTmp);
            }
        } catch (Exception e) {
        ...
        getLogger().info("Finish Reading Unauth File.");
    }

    private void readXLSX(String param5, UnauthTmp unauthTmp, UnauthTmpDao unauthTmpDao) throws FIXException, FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        ...
        processSheet(styles, strings, stream, unauthTmp, unauthTmpDao);
        stream.close();
    }

    public void processSheet(StylesTable styles, ReadOnlySharedStringsTable strings, InputStream sheetInputStream, UnauthTmp unauthTmp, UnauthTmpDao unauthTmpDao) throws FIXException, IOException, ParserConfigurationException, SAXException {
        int minColumns = -1;
        InputSource sheetSource = new InputSource(sheetInputStream);
        SAXParserFactory saxFactory = SAXParserFactory.newInstance();
        SAXParser saxParser = saxFactory.newSAXParser();
        XMLReader sheetParser = saxParser.getXMLReader();
        ContentHandler handler = new MyXSSFSheetHandler(styles, strings, minColumns, System.out, unauthTmp, unauthTmpDao);
    ...
    private void loadConfig(String confFile) throws FileNotFoundException, IOException {
        ...
        InputStream in = UnauthWorker.class.getClassLoader().getResourceAsStream(confFile);
        properties.load(in);        
    ...
    class MyXSSFSheetHandler extends DefaultHandler {
        ...
        private UnauthTmp ufcr;
        private UnauthTmpDao ufcrDao;

        public MyXSSFSheetHandler(StylesTable styles, ReadOnlySharedStringsTable strings, int cols, PrintStream target, UnauthTmp unauthTmp, UnauthTmpDao unauthTmpDao) {
            ...
            this.ufcr = unauthTmp;
            this.ufcrDao = unauthTmpDao;
            this.ufcr = new UnauthTmp();
        }
    }       
