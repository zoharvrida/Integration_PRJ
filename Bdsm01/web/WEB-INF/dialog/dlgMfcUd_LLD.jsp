...
<s:url var="urlGrid" action="28302_list" namespace="/json" escapeAmp="false">
	<s:param name="noCif">${param.noCif}</s:param>
	<s:param name="noUd">${param.noUd}</s:param>
</s:url>
...
>
	<sjg:gridColumn name="COD_CUST_ID" index="COD_CUST_ID" title="CIF No" sortable="true" width="100" hidden="true" />
	<sjg:gridColumn name="NOUD" index="NOUD" title="UD No" sortable="true" width="400"/>
	<sjg:gridColumn name="CATEGORY_UD" index="CATEGORY_UD" title="Category" sortable="true" width="40"/>
	...
	<sjg:gridColumn name="DT_ISSUED" formatter="date" width="100" formatoptions="{newformat : 'd/m/Y', srcformat : 'Y-m-d H:i:s'}" 
			index="DT_ISSUED" title="Issued Date" sortable="true" hidden="true" />
	<sjg:gridColumn name="DTEXPIRY" formatter="date" width="100" formatoptions="{newformat : 'd/m/Y', srcformat : 'Y-m-d H:i:s'}"
...
<script type="text/javascript">
	$(function() {
		...
		$("#gridtable").subscribe("selectRow", function(event, data) {
			$("#" + dlgParams.noCif).val($(data).jqGrid("getCell", event.originalEvent.id, "COD_CUST_ID"));
			$("#" + dlgParams.noUd).val($(data).jqGrid("getCell", event.originalEvent.id, "NOUD"));
			$("#" + dlgParams.catUD).val($(data).jqGrid("getCell", event.originalEvent.id, "CATEGORY_UD"));
		...
		$("#gridtable").subscribe("gridComplete", function(event, data) {
			$("#ph-dlg").dialog({
				width : Math.min(screen.availWidth - 20, 700),
				height : $("#gbox_gridtable").height() + $("div.ui-dialog-titlebar.ui-widget-header").height() + 44
			});
			$("#ph-dlg").dialog({
				position : "center"
			});
