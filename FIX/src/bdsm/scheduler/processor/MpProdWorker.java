...
import bdsm.scheduler.dao.TmpMpProdDao;
 ...
 * @author v00020841
 */
public class MpProdWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process register product mitra pasti has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process register product mitra pasti has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public MpProdWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        ...
        TmpMpProdDao tmpMpProdDao = new TmpMpProdDao(session);
...
//                    dateFormatter.parse(dateInFilename);
            ...
            this.readExcel(param5, configFile, tmpMpProdDao);
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            tmpMpProdDao.runValidate(batchNo);
            ...
            tmpMpProdDao.runProcess(batchNo);
            ...
            tmpMpProdDao.runReject(batchNo);
            this.getLogger().info("Register FixQXtract");
            fixQXtract = new FixQXtract();
    ...
    private void readExcel(String param5, String configFile, TmpMpProdDao tmpMpProdDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, tmpMpProdDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, tmpMpProdDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMpProdDao tmpMpProdDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMpProd tmpMpProd;
        TmpMpPK tmpMPPK;
        String[] propertyNames = {"codProd", "cmd"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            tmpMpProd = new TmpMpProd();
            tmpMPPK = new TmpMpPK();
            tmpMPPK.setBatchNo(context.get(MapKey.batchNo).toString());
            tmpMPPK.setRecordId(recordId);
            tmpMpProd.setCompositeId(tmpMPPK);
            tmpMpProd.setIdCreatedBy("BDSM");
            tmpMpProd.setIdCreatedSpv("BDSM");
            tmpMpProd.setDtmCreated(SchedulerUtil.getTime());
            tmpMpProd.setDtmUpdated(SchedulerUtil.getTime());
            tmpMpProd.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(tmpMpProd, propertyNames)) {
                    tmpMpProdDao.insert(tmpMpProd);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                tmpMpProd.setFlagStatus(StatusDefinition.REJECTED);
                tmpMpProd.setStatusReason(ex.toString());
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpMpProdDao tmpMpProdDao) {
