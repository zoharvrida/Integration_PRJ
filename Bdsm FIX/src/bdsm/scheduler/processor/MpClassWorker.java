...
import bdsm.scheduler.dao.TmpMpClassDao;
...
import bdsm.scheduler.model.TmpMpClass;
 ...
 * @author v00020841
 */
public class MpClassWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process mp_class product mitra pasti has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process mp_class product mitra pasti has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public MpClassWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        ...
        TmpMpClassDao mpClassDao = new TmpMpClassDao(session);
...
//                    dateFormatter.parse(dateInFilename);
            ...
            this.readExcel(param5, configFile, mpClassDao);
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            mpClassDao.runValidate(batchNo);
    ...
    private void readExcel(String param5, String configFile, TmpMpClassDao mpClassDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, mpClassDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, mpClassDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMpClassDao mpClassDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMpClass tmpMpClass;
        TmpMpPK tmpMPPK;
        String[] propertyNames = {"codClass","codProd","codMis","namClass", "cmd"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            tmpMpClass = new TmpMpClass();
            tmpMPPK = new TmpMpPK();
            tmpMPPK.setBatchNo(context.get(MapKey.batchNo).toString());
            tmpMPPK.setRecordId(recordId);
            tmpMpClass.setCompositeId(tmpMPPK);
            tmpMpClass.setIdCreatedBy("BDSM");
            tmpMpClass.setIdCreatedSpv("BDSM");
            tmpMpClass.setDtmCreated(SchedulerUtil.getTime());
            tmpMpClass.setDtmUpdated(SchedulerUtil.getTime());
            tmpMpClass.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(tmpMpClass, propertyNames)) {
                    mpClassDao.insert(tmpMpClass);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                tmpMpClass.setFlagStatus(StatusDefinition.REJECTED);
                tmpMpClass.setStatusReason(ex.toString());
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpMpClassDao mpClassDao) {
