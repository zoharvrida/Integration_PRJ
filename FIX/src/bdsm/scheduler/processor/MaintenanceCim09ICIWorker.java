...
import bdsm.scheduler.dao.TmpMaintenanceCim09ICIDao;
...
import bdsm.scheduler.model.TmpMaintenanceCim09ICIPK;
 ...
 * @author v00020841
 */
public class MaintenanceCim09ICIWorker extends BaseProcessor {
     ...
     * @param context
     */
    public MaintenanceCim09ICIWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_cim09ICI.properties";
        ...
        TmpMaintenanceCim09ICIDao cim09ICIDao = new TmpMaintenanceCim09ICIDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            this.readExcel(param5, configFile, cim09ICIDao); //Must Change
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            cim09ICIDao.runValidate(batchNo);
            ...
            cim09ICIDao.runProcess(batchNo);
    ...
    private void readExcel(String param5, String configFile, TmpMaintenanceCim09ICIDao cim09ICIDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, cim09ICIDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, cim09ICIDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMaintenanceCim09ICIDao cim09ICIDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMaintenanceCim09ICI cim09ICI;
        TmpMaintenanceCim09ICIPK cim09ICIPK;
        String[] propertyNames = {"compositeId.cif", "statusIC", "signTypIC", "ethnicIC", "countryResidenIC", "sexIC", "profes",
            "criteria", "designationIC", "birth", "nationalityIC", "educationIC", "marital", "spouse"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            cim09ICIPK = new TmpMaintenanceCim09ICIPK();
            cim09ICIPK.setBatchiId(context.get(MapKey.batchNo).toString());
            cim09ICIPK.setRecordId(recordId);
            cim09ICI = new TmpMaintenanceCim09ICI();
            cim09ICI.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            cim09ICI.setCompositeId(cim09ICIPK);
            try {
                if (xr.nextRow(cim09ICI, propertyNames)) {
                    if (cim09ICI.getBirth() != null) {
                        try {
                            cim09ICI.setDobIC(dateFormatter2.parse(cim09ICI.getBirth()));
                        } catch (Exception e) {
                            cim09ICI.setStatusReason("Invalid date, format must be [DD-MM-YYYY]");
                            cim09ICI.setFlagStatus(StatusDefinition.REJECTED);
                        }
                    }
                    if (cim09ICI.getProfes() != null) {
                        try {
                            cim09ICI.setProfesCodeIC(Integer.parseInt(cim09ICI.getProfes()));
                        } catch (Exception e) {
                            cim09ICI.setStatusReason("Reject, [Profession Code Must Be Number]");
                            cim09ICI.setFlagStatus(StatusDefinition.REJECTED);
                        }
                    } 
                    if (cim09ICI.getMarital() != null) {
                        try {
                            cim09ICI.setMaritalStatus(Integer.parseInt(cim09ICI.getMarital()));
                        } catch (Exception e) {
                            cim09ICI.setStatusReason("Reject, [Marital Status Must Be Number]");
                            cim09ICI.setFlagStatus(StatusDefinition.REJECTED);
                        }
                    } 
                    this.getLogger().info("criteria : "+cim09ICI.getCriteria());
                    if (cim09ICI.getCriteria() != null) {
                        try {
                            cim09ICI.setSrcCrteriaIC(Integer.parseInt(cim09ICI.getCriteria()));
                        } catch (Exception e) {
                            cim09ICI.setStatusReason("Reject, [Search Criteria Must Be Number]");
                            cim09ICI.setFlagStatus(StatusDefinition.REJECTED);
                        }
                    } 
                    if (cim09ICI.getSpouse() != null) {
                        try {
                            cim09ICI.setNoOfSpouseIC(Integer.parseInt(cim09ICI.getSpouse()));
                        } catch (Exception e) {
                            cim09ICI.setStatusReason("Reject, [No of Spouse Must Be Number]");
                            cim09ICI.setFlagStatus(StatusDefinition.REJECTED);
                        }
                    }
                    cim09ICIDao.insert(cim09ICI);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                cim09ICI.setStatusReason(ex.toString());
                cim09ICI.setFlagStatus(StatusDefinition.REJECTED);
                cim09ICIDao.insert(cim09ICI);
            }
        }
    ...
    private void readXLSX(String param5, String configFile, TmpMaintenanceCim09ICIDao cim09ICIDao) {
