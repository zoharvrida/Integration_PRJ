...
import bdsm.scheduler.dao.TmpBigBillMastCustomerDao;
...
import bdsm.scheduler.model.TmpBigBillMastCustomer;
 ...
 * @author v00020841
 */
public class BigBillMastCustomerWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process register customer big bill pln has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process to register customer big bill pln has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public BigBillMastCustomerWorker(Map<String, ? extends Object> context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_bigBillMastCustomer.properties";
        ...
        TmpBigBillMastCustomerDao bigBillMastCustomerDao = new TmpBigBillMastCustomerDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            try {
                this.readExcel(param5, configFile, bigBillMastCustomerDao); //Must Change
            }
            catch (FIXException fe) {
                String message = fe.getMessage();
                
                if (message.startsWith("Over of maximum records")) {
                    ...
                    this.fixQXtract.setParam2(this.context.get(MapKey.emailSender).toString());
                    this.fixQXtract.setParam4("Error, " + message);
                    this.fixQXtract.setParam5("");
                }
                else
                    throw fe;
            ...
            bigBillMastCustomerDao.runValidate(batchNo);
            ...
            bigBillMastCustomerDao.runProcess(batchNo);
            ...
            bigBillMastCustomerDao.runReject(batchNo);
            fixQXtract = new FixQXtract();
            fClassConfig = classConfigDao.get(getClass().getName(), sourceProcess, StatusDefinition.REJECTED).get(0);
            idSchedulerXtract = fClassConfig.getIdScheduler();
            fixQXtract.setIdScheduler(idSchedulerXtract);
    ...
    private void readExcel(String param5, String configFile, TmpBigBillMastCustomerDao bigBillMastCustomerDao) throws Exception {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, bigBillMastCustomerDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, bigBillMastCustomerDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpBigBillMastCustomerDao bigBillMastCustomerDao) throws Exception {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpBigBillMastCustomer mastCustomer;
        String[] propertyNames = {"custVaNo","custName","custAddr","custNpwp","custPhone","custFax","custIdPel",
                                  "custAverageInv","custStartPay","custPicName","custPicHp","custPicEmail","aoCode",
                                  "lob","aoWil","aoBranch","aoHp","aoEmail","cmd"};
        ...
        if (numberOfRecords > PropertyPersister.BIGBILL_MAX_RECORDS_REGISTRATION)
            throw new FIXException("Over of maximum records " + PropertyPersister.BIGBILL_MAX_RECORDS_REGISTRATION + ": " + numberOfRecords);
        

        xr.reset();
        while (xr.hasNextRow()) {
            mastCustomer = new TmpBigBillMastCustomer();
            mastCustomer.setBatchNo(context.get(MapKey.batchNo).toString());
            mastCustomer.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(mastCustomer, propertyNames)) {
                    if (mastCustomer.getCustAverageInv() == null)
                        mastCustomer.setCustAverageInv("0");
                    
                    mastCustomer.setCustAvgInv(new BigDecimal(mastCustomer.getCustAverageInv()));
                    bigBillMastCustomerDao.insert(mastCustomer);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                mastCustomer.setStatusReason("Invalid data for field cust_avg_inv");
                mastCustomer.setFlagStatus(StatusDefinition.REJECTED);
                bigBillMastCustomerDao.insert(mastCustomer);
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpBigBillMastCustomerDao bigBillMastCustomerDao) throws Exception {
        ...
        while (xr.hasNextRow()) {
            ...
            try {
                ...
                this.getLogger().error(ex, ex);
                mastCustomer.setStatusReason(ex.toString());
                mastCustomer.setFlagStatus(StatusDefinition.REJECTED);
                bigBillMastCustomerDao.insert(mastCustomer);
            }
        }
