...
<s:if test="%{#request.requestMethod.equals('POST')}">
	<s:url var="ajaxUpdateTitle" action="23206_title" />
	<s:url var="ajaxUpdateButtons" action="23206_buttons" />
	...
	<s:form id="frmDlgAuth" action="dlg">
		...
		<s:hidden name="idMenu" value="23206" />
	</s:form>
	<sj:a id="aDlgAuth" formIds="frmDlgAuth" targets="ph-dlg" cssClass="ui-helper-hidden" />
	
	<s:form id="frmMain" name="frmMain" action="23206" method="post" validate="true">
		<s:select 
			name="batchNo"
			list="mapFilename"
			emptyOption="true"
	...
	<s:url var="urlTab" action="23206Tab_none" />
	
	<sj:tabbedpanel id="tabbedPanelHeader" disabled="true" cssStyle="max-width:950px">
		<sj:tab id="tabHeader" href="%{urlTab}" label="SKNNG Outward Debit Header" />
	</sj:tabbedpanel>
	...
	<fieldset id="fsData" class="ui-widget-content ui-corner-all ui-helper-hidden">
		...
		<s:form id="frmData" name="frmData" action="23206_execute" method="post" theme="css_xhtml">
			<table>
				<tbody>
					<tr>
						<td>
			...
			<sj:tabbedpanel id="tabbedPanelDetail" disabled="true" cssStyle="max-width:900px">
				<sj:tab id="tabDetail" href="%{urlTab}" label="SKNNG Outward Debit Detail" />
			</sj:tabbedpanel>
	...
	<script type="text/javascript">
		$(function() {
			...
			$("#btnFind").click(function() {
				...
				$("#tabbedPanelHeader").tabs("url", 0, "23206Tab_execute.action");
