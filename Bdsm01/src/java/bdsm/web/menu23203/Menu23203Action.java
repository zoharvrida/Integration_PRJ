 ...
 */
package bdsm.web.menu23203;

import bdsm.web.menu23202.*;
 ...
 * @author bdsm
 */
public class Menu23203Action extends BaseContentAction {

    private String APPROVE_ACTION = "SknNgSMODebit_update.action";
    private String REJECT_ACTION ="SknNgSMODebit_insert.action";
    ...
    public String doAdd() {
        ...
        this.getLogger().info("parent ID : " + this.recIds);
        this.getLogger().info("isApprove : " + this.getIsApprove());
		
		String DateS = null;
		try {
			DateS = SchedulerUtil.getDate("yyMMdd");
		} catch (ParseException ex) {
			Logger.getLogger(Menu23203Action.class.getName()).log(Level.SEVERE, null, ex);
		}
        ...
        map.put("idMaintainedBy", idMaintainedBy);
        map.put("idMaintainedSpv", idMaintainedSpv);
        map.put("token", BdsmUtil.generateToken(getTokenKey(), getTzToken()));

        if ("true".equals(this.getIsApprove())) {
            this.setMode("1");
            map.put("mode", this.getMode());
        result = HttpUtil.request(getBdsmHost() + APPROVE_ACTION, map);
        } else {
            this.setMode("2");
            map.put("mode", this.getMode());
...
/*
        ...
        getLogger().info("batchNo : " + batchNo);
        getLogger().info("cdBranch : " + cdBranch);
        ...
        if (reqResult.equals(ActionSupport.ERROR)) {
            addActionError("Fail Approve Batch " + batchNo);
        } else if (reqResult.equals(ActionSupport.SUCCESS)) {
            addActionMessage("Batch " + batchNo + " Approved Successfully");
        }
//
        return ActionSupport.SUCCESS;
