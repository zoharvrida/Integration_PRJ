...
<s:if test="%{#request.requestMethod.equals('POST')}">
	<s:url var="ajaxUpdateTitle" action="23202_title" />
	<s:url var="ajaxUpdateButtons" action="23202_buttons" />
	...
	<s:form id="frmMain" name="frmMain" action="23202" method="post" validate="true">
		<s:url id="getListType" action="json/23202_getListType" escapeAmp="false">
		</s:url>
		<s:textfield
			name="IDBATCH" 
			size="10" 
	...
	<s:url var="urlTab1" action="23202Tab_none">
		<param name="prmBatchNo" value="" />
	</s:url>
	<s:url var="urlTab2" action="23202Tab_none">
		<param name="prmBatchNo" value="" />
	</s:url>

	<sj:tabbedpanel id="tabbedPanel" disabled="true">
		<sj:tab id="tabinquiry" href="%{urlTab1}" label="SKNNG Incoming Credit Inquiry" />
		<sj:tab id="tabapproval" href="%{urlTab2}" label="SKNNG Incoming Credit Approval" />
	</sj:tabbedpanel>
			...
			$("#btnFind").click(function() {
				...
				if ($("#gRPID").val() == '-1')
					...
					if (act == "inquiry") {
						...
						$("#tabbedPanel").tabs("url", 0, "23202Tab_execute.action");
						$("#tabbedPanel").tabs("load", 0);
						//alert(0);
					}
					else if (act == "approve") {
						$("#tabbedPanel").tabs({selected: 1});
						$("#tabbedPanel").tabs("url", 1, "23202Tab_input.action");
