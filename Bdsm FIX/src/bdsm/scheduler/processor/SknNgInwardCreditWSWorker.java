...
public class SknNgInwardCreditWSWorker extends BaseProcessor {
	...
	protected boolean doExecute() throws Exception {
		...
		TmpSknngInOutCreditDetailDAO detailDAO = new TmpSknngInOutCreditDetailDAO(this.session);
		BaBankMastDAO baBankMastDAO = new BaBankMastDAO(this.session);
		List<TmpSknngInOutCreditHeader> listHeader = null;
		TmpSknngInOutCreditDetail dbDetail = null;
		...
		boolean isHeaderNeedMoreCheck = false;
			...
			while ((line = reader.readLine()) != null) {
				...
				switch (line.charAt(0)) {
					case '0' : {
							...
							listHeader = headerDAO.listBy_Tanggal_IdPeserta_Kota_BatchId(
									modelHeader.getTanggalBatch(), modelHeader.getIdentitasPesertaPengirimPenerus(), modelHeader.getSandiKotaAsal(), modelHeader.getBatchId()
								);
								if ((listHeader == null) || (listHeader.size() == 0))
									isValid = true;
								else {
									for (int i=0; i<listHeader.size(); i++) {
										TmpSknngInOutCreditHeader h = listHeader.get(0);
										
										if ((h.getJumlahRecords().trim().equals(modelHeader.getJumlahRecords().trim())) && 
												(h.getTotalNominal().trim().equals(modelHeader.getTotalNominal().trim().replace(',', '.')))
											)
												isHeaderNeedMoreCheck = true;
										else
											listHeader.remove(i--);
									}
									
									if (!isHeaderNeedMoreCheck)
										isValid = true;
									else
										lineHeader = line;
								}
					...
					case '1' : {
							if (isHeaderNeedMoreCheck) {
								for (TmpSknngInOutCreditHeader h: listHeader) {
									dbDetail = detailDAO.getFullListDetails(h.getCompositeId().getBatchNo(), java.util.Arrays.asList(h.getCompositeId().getParentRecordId())).get(0);
									modelDetail = service.parseToInOutDetail(line, null, null, modelHeader);
									
									if (dbDetail.getSor().equals(modelDetail.getSOR())) {
										isValid = false;
										break;
									}
								...
								isHeaderNeedMoreCheck = false;
								
								if (isValid)
									writeln(buffWriter, lineHeader);
							}
