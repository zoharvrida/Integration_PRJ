...
public class DDFInterfaceValidation extends TXTReaderWorker {
    ...
    protected String HeaderCIF(String[] detail, Integer total) {
        ...
        FcrBaBankMastDao badao = new FcrBaBankMastDao(session);
        ...
        String dateFcomp = df.format((badao.get().getDatProcess()));
    ...
    protected String HeaderCASA(String[] detail, Integer total) {
        ...
        String dateF = null;
        String dateFcomp = null;
        try {
            SimpleDateFormat df = new SimpleDateFormat(StatusDefinition.Patternyyyymmdd);
            dateF = df.format(Date.valueOf(detail[1]));
            getLogger().debug("dateF" + " " + dateF);
            dateFcomp = df.format((badao.get().getDatProcess()));
            getLogger().debug(dateF + " " + dateFcomp);
            getLogger().debug("detail[1]" + " " + detail[1]);
            getLogger().debug("dateFcomp" + " " + dateFcomp);
        } catch (Exception e) { 
            getLogger().debug("DATE_ERR" + e,e);
            
            try {
                getLogger().debug("DETECT" + " " + dao.bankMast());
                SimpleDateFormat df = new SimpleDateFormat(StatusDefinition.patternddMMyyyy);
                dateFcomp = df.format(Date.valueOf(dao.bankMast()));
            } catch (Exception f) {
                getLogger().debug("DATE_ERROR" + f,f);
            }
        }
        ...
        if (!dao.get(idBatch).isEmpty()) {
            try {
                ...
                getLogger().info("ERROR SQL :" + ex,ex);
            ...
            for (Integer i = 0; i < detail.length; i++) {
                ...
                try {
                    ...
                    getLogger().info("EXCEPTION :" + this.flags + " " + e,e);
                    this.flags = StatusDefinition.YES;
                }
            ...
            if (!dateF.equals(dateFcomp)) {
                if (StatusDefinition.YES.equals(mandatory[1])) {
                    ...
                    getLogger().debug("dateF" + " " + dateF);
                    getLogger().debug("dateFcomp" + " " + dateFcomp);
                    getLogger().debug("mandatory[1]" + " " + mandatory[1]);
                }
                try {
