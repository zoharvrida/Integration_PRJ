/**
 * ************************************************************************
 * copyright file="ServiceRequestBase.java" company="Microsoft" Copyright (c)
...
abstract class ServiceRequestBase {
    ...
    private static final Logger logger = Logger.getLogger(ServiceRequestBase.class);
    // Private Constants
     ...
     * @throws ServiceXmlDeserializationException the service xml
    ...
    /**
     * * Writes XML attributes. Subclass will override if it has XML
     * attributes.
     *
     * @param writer The writer.
     * @throws ServiceXmlSerializationException the service xml serialization
    ...
    protected void throwIfNotSupportedByRequestedServerVersion()
        ...
        if (this.service.getRequestedServerVersion().ordinal() < this.getMinimumRequiredServerVersion().ordinal()) {
            throw new ServiceVersionException(String.format(
                    Strings.RequestIncompatibleWithRequestVersion, this.getXmlElementName(), this.getMinimumRequiredServerVersion()));
        }
    }
    ...
    protected void writeToXml(EwsServiceXmlWriter writer) throws Exception {
        ...
        if (this.service.getPreferredCulture() != null) {
            writer.writeElementValue(XmlNamespace.Types,
                    XmlElementNames.MailboxCulture, this.service.getPreferredCulture().getDisplayName());
        }
        ...
        if (this.getService().getDateTimePrecision().ordinal() != DateTimePrecision.Default.ordinal()) {
            writer.writeElementValue(XmlNamespace.Types,
                    XmlElementNames.DateTimePrecision, this.getService().getDateTimePrecision().toString());
        }
     ...
     * @throws InterruptedException @returns The Request stream
     */
     ...
     * decompress content).
     *
     * @param request HttpWebRequest object from which response stream can be
     * read.
    ...
    protected Object readResponse(EwsServiceXmlReader ewsXmlReader)
            throws Exception {
        logger.debug("RESPREAD :" + ewsXmlReader);
    ...
    protected void readPreamble(EwsServiceXmlReader ewsXmlReader)
            throws Exception {
        logger.debug("XMLREADER :" + ewsXmlReader);
     ...
     * @param asyncResult An IAsyncResult that references the asynchronous
     * request. @returns An HttpWebResponse instance
     */
     ...
     *
     * @param request The specified HttpWebRequest @returns An HttpWebResponse
     * instance
     */
    protected HttpWebRequest getEwsHttpWebResponse(
        ...
        try {
            ...
            logger.error("HttpErrorException " + ex, ex);
            // Wrap exception if the above code block didn't throw
            throw new ServiceRequestException(String.format(
                    Strings.ServiceRequestFailed, ex.getMessage()), ex);
        } catch (IOException e) {
