...
public class SknNgSPKWebService {
	...
	public static String getDKEKreditInwardBulk(Spkintf interface_, String BIC, String auth, String period, String cityCode) throws Exception {
		LOGGER.debug("[DEBUG] SknWsClient.getDKEKreditInwardBulk() start\n");
		
		byte[] bResult = interface_.getTPKWebServiceImplPort().getDKEKreditInwardBulk(BIC, auth, BIC, period, cityCode);
		
		String result = isEncrypted()? decryptMessage(bResult, ENCRYPTION): new String(bResult);
		LOGGER.debug("[DEBUG] SknWsClient.getDKEKreditInwardBulk() result: \n" + result);
	...
	public static String sendDKEPengembalianBulk(Spkintf interface_, String BIC, String auth, String data) throws Exception {
		...
		if (isEncrypted())
			LOGGER.debug("result encrypted: " + bdsm.util.EncryptionUtil.byteArrayToHexString(message));
		String result = isEncrypted()? decryptMessage(bResult, ENCRYPTION): new String(bResult);
	...
	public static boolean checkService(String type, String[] errorMessage) {
		try {
			if (SknNgSPKWebService.CONVENT_BIC.equals(type)) 
				SknNgSPKWebService.getDaftarAlasanPenolakan(SknNgSPKWebService.CONVENT_SERVICE, SknNgSPKWebService.CONVENT_BIC, SknNgSPKWebService.CONVENT_AUTH);
			else if (SknNgSPKWebService.SYARIAH_BIC.equals(type))
				SknNgSPKWebService.getDaftarAlasanPenolakan(SknNgSPKWebService.SYARIAH_SERVICE, SknNgSPKWebService.SYARIAH_BIC, SknNgSPKWebService.SYARIAH_AUTH);
		}
		catch (Exception ex) {
			LOGGER.error(ex, ex);
