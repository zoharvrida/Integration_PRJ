...
class EwsXmlReader {
    ...
    private Logger logger = Logger.getLogger(EwsXmlReader.class);
	/** The xml reader. */
	...
	private void internalReadElement(XmlNamespace xmlNamespace,
		...
		if (xmlNamespace == XmlNamespace.NotSpecified) {
			...
			if ((!this.getLocalName().equals(localName)) ||
                ...
                logger.debug("VAR :" + localName);
				logger.debug("NAMESPACE :" + getNamespaceUri());
                logger.debug("GET VAR :" + getLocalName());
	...
	public void read(XMLNodeType nodeType) throws Exception {
		this.read();
        logger.debug("COMPARE :" + this.getNodeType() + " - " + nodeType);
	...
	throws Exception {
        logger.debug("readElement 1 :" + namespacePrefix + " - " + localName);
	...
	throws Exception {
		logger.debug("readElement 2 :" + xmlNamespace + " - " + localName);
