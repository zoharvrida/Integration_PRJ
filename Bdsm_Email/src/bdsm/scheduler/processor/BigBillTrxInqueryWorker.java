...
import bdsm.scheduler.dao.TmpBigBillTrxInqDao;
 ...
 * @author v00020841
 */
public class BigBillTrxInqueryWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process inquiry big bill pln  has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process inquiry big bill pln has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";

    public BigBillTrxInqueryWorker(Map<String, ? extends Object> context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_bigBillTrxInq.properties";
        ...
        TmpBigBillTrxInqDao bigBillTrxInqDao = new TmpBigBillTrxInqDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            try {
                this.readExcel(param5, configFile, bigBillTrxInqDao); //Must Change
            }
            ...
            bigBillTrxInqDao.runValidate(batchNo);
            ...
            bigBillTrxInqDao.runProcess(batchNo);
            ...
            bigBillTrxInqDao.runReject(batchNo);
            fixQXtract = new FixQXtract();
            fClassConfig = classConfigDao.get(getClass().getName(), sourceProcess, StatusDefinition.REJECTED).get(0);
            idSchedulerXtract = fClassConfig.getIdScheduler();
            fixQXtract.setIdScheduler(idSchedulerXtract);
    ...
    private void readExcel(String param5, String configFile, TmpBigBillTrxInqDao bigBillTrxInqDao) throws Exception {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, bigBillTrxInqDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, bigBillTrxInqDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpBigBillTrxInqDao bigBillTrxInqDao) throws Exception {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpBigBillTrx bigBillTrxInq;
        String[] propertyNames = {"custIdPel","typeTrx"};
        ...
        xr.reset();
        while (xr.hasNextRow()) {
            bigBillTrxInq = new TmpBigBillTrx();
            bigBillTrxInq.setBatchNo(context.get(MapKey.batchNo).toString());
            bigBillTrxInq.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(bigBillTrxInq, propertyNames)) {
                    bigBillTrxInqDao.insert(bigBillTrxInq);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                bigBillTrxInq.setStatusReason(ex.toString());
                bigBillTrxInq.setFlagStatus(StatusDefinition.REJECTED);
                bigBillTrxInqDao.insert(bigBillTrxInq);
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpBigBillTrxInqDao bigBillTrxInqDao) throws Exception {
