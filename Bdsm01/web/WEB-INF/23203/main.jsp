...
<s:if test="%{#request.requestMethod.equals('POST')}">
	<s:url var="ajaxUpdateTitle" action="23203_title" />
	<s:url var="ajaxUpdateButtons" action="23203_buttons" />
	...
	<s:form id="frmMain" name="frmMain" action="23203" method="post" validate="true">
		<s:url id="getListType" action="json/23203_getListType" escapeAmp="false">
		</s:url>
		<s:textfield
			name="IDBATCH" 
			size="10" 
	...
	<s:url var="urlTab1" action="23203Tab_none">
		<param name="prmBatchNo" value="" />
	</s:url>
	<s:url var="urlTab2" action="23203Tab_none">
		<param name="prmBatchNo" value="" />
	</s:url>
			...
			$("#btnFind").click(function() {
				...
				if ($("#gRPID").val() == '-1')
					...
					if (act == "inquiry") {
						...
						$("#tabbedPanel").tabs("url", 0, "23203Tab_execute.action");
						$("#tabbedPanel").tabs("load", 0);
						//alert(0);
					}
					else if (act == "approve") {
						$("#tabbedPanel").tabs({selected: 1});
						$("#tabbedPanel").tabs("url", 1, "23203Tab_input.action");
