...
<s:if test="%{#request.requestMethod.equals('POST')}">
	<s:url var="ajaxUpdateTitle" action="23205_title" />
	<s:url var="ajaxUpdateButtons" action="23205_buttons" />
	...
	<s:form id="frmMain" name="frmMain" action="23205" focusElement="frmMain_namTemplateSearch" method="post" validate="true">
		<s:url var="codeNoURL" action="23205_list.action?action=select&page=1&rows=100" namespace="/json" />
		<sj:select
			id="frmMain_mode"
			name="mode"
			href="%{codeNoURL}"
	...
	<s:url var="urlTab1" action="23205Tab_none">
		<param name="prmBatchNo" value="" />
	</s:url>
	<s:url var="urlTab2" action="23205Tab_none">
		<param name="prmBatchNo" value="" />
	</s:url>
			...
			$("#btnFind").click(function() {
				...
				if ($("#frmMain_codeNo").val() == '-1')
					...
					if (act == "inquiry") {
						...
						$("#tabbedPanel").tabs("url", 0, "23205Tab_execute.action");
						$("#tabbedPanel").tabs("load", 0);
					}
					else if (act == "approve") {
						$("#tabbedPanel").tabs({selected : 1});
						$("#tabbedPanel").tabs("url", 1, "23205Tab_input.action");
