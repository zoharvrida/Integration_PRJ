...
abstract class SimpleServiceRequestBase extends ServiceRequestBase {
    ...
    private Logger logger = Logger.getLogger(SimpleServiceRequestBase.class);
	/** 
		...
		try {
			...
			if (this.getService().isTraceEnabledFor(TraceFlags.EwsResponse)) {
				...
				try {
                    ...
                    logger.debug("SERV EXP :" + e,e);
                    logger.debug("SERVICE :" + serviceResponse);
                }

			}
		} catch (HttpException e) {
            logger.debug("HTTP :" + e,e);
