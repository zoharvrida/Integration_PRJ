 ...
 * @author v00020841
 */
public class BigBillTrxFlaggingWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process flagging big bill pln  has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process to flagging big bill pln has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public BigBillTrxFlaggingWorker(Map<String, ? extends Object> context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_bigBillTrxFlagging.properties";
        ...
        TmpBigBillTrxFlgDao bigBillTrxFlgDao = new TmpBigBillTrxFlgDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            try {
                this.readExcel(param5, configFile, bigBillTrxFlgDao); //Must Change
            }
            ...
            bigBillTrxFlgDao.runValidate(batchNo);
            ...
            bigBillTrxFlgDao.runProcess(batchNo);
            ...
            bigBillTrxFlgDao.runReject(batchNo);
            fixQXtract = new FixQXtract();
            fClassConfig = classConfigDao.get(getClass().getName(), sourceProcess, StatusDefinition.REJECTED).get(0);
            idSchedulerXtract = fClassConfig.getIdScheduler();
            fixQXtract.setIdScheduler(idSchedulerXtract);
    ...
    private void readExcel(String param5, String configFile, TmpBigBillTrxFlgDao bigBillTrxFlgDao) throws Exception {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, bigBillTrxFlgDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, bigBillTrxFlgDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpBigBillTrxFlgDao bigBillTrxFlgDao) throws Exception {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpBigBillTrx bigBillTrxFlagging;
        String[] propertyNames = {"custIdPel", "typeTrx", "amountFlagging"};
        ...
        if (numberOfRecords > PropertyPersister.BIGBILL_MAX_RECORDS_PAYMENT)
            throw new FIXException("Over of maximum records " + PropertyPersister.BIGBILL_MAX_RECORDS_PAYMENT + ": " + numberOfRecords);
        
        
        xr.reset();
        while (xr.hasNextRow()) {
            bigBillTrxFlagging = new TmpBigBillTrx();
            bigBillTrxFlagging.setBatchNo(context.get(MapKey.batchNo).toString());
            bigBillTrxFlagging.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            try {
                if (xr.nextRow(bigBillTrxFlagging, propertyNames)) {
                    if (bigBillTrxFlagging.getAmountFlagging() == null)
                        throw new NumberFormatException("Blank or invalid format Amount");
                    bigBillTrxFlgDao.insert(bigBillTrxFlagging);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                bigBillTrxFlagging.setStatusReason(ex.toString());
                bigBillTrxFlagging.setFlagStatus(StatusDefinition.REJECTED);
                bigBillTrxFlgDao.insert(bigBillTrxFlagging);
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpBigBillTrxFlgDao bigBillTrxFlgDao) throws Exception {
