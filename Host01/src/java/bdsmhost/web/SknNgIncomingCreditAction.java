		...
		try {
			...
			for (TmpSknngInOutCreditHeader header : fullHeader) {
				...
				for (int i = 0; i < fullDetails.size();) {
					TmpSknngInOutCreditDetail detail = fullDetails.get(i);
					if (header.getCompositeId().getParentRecordId().compareTo(detail.getCompositeId().getParentRecordId()) == 0) {
						...
						if (detail != null) {
							...
							if (!"R".equalsIgnoreCase(detail.getFlgAuth())) {
								...
								modelG2Pk.setParentRecordId(header.getCompositeId().getParentRecordId());
								...
								modelhPk.setParentRecordId(header.getCompositeId().getParentRecordId());
								...
								try {
									modelPm.setCod_bank(G1dao.getCod_Bi(detail.getIdentitasPesertaPenerimaAkhir()));
								} catch (Exception e) {
									getLogger().info("CODBANK : " + detail.getIdentitasPesertaPenerimaAkhir() + " NOT FOUND");
									modelPm.setCod_bank("0");
								}
								try {
								...
								try {
									detailsG1.setClearingDate((header.getTanggalBatch()).substring(2, 10).replaceAll("-", "")); // header 4
									detailsG1.setParentRecid(header.getCompositeId().getParentRecordId());
									...
									/* If from bulk-to-individual-conversion transaction */
				...
				try {
					...
					getLogger().debug("PARENT :" + header.getCompositeId().getParentRecordId());
					TmpSknngInOutCreditFooter getFooter = fDaog2.get(batchNo, header.getCompositeId().getParentRecordId());
					//getLogger().info("LOGGER :" + rejectCount + " " + successCount);
					getFooter.setTreject(rejectCount);
					getFooter.setTsuccess(successCount);
