...
import bdsm.scheduler.dao.PmSwiftNostroDao;
...
import bdsm.scheduler.model.TmpPmSwiftNostro;
import bdsm.scheduler.model.TmpPmSwiftNostroPK;
 ...
 * @author v00020841
 */
public class MaintenancePmSwitNostro extends BaseProcessor {
     ...
     * @param context
     */
    public MaintenancePmSwitNostro(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_pmswiftnostro.properties";
        ...
        PmSwiftNostroDao pmSwitNostroDao = new PmSwiftNostroDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            this.readExcel(param5, configFile, pmSwitNostroDao); //Must Change
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            pmSwitNostroDao.runValidate(batchNo);
            ...
            pmSwitNostroDao.runProcess(batchNo);
            ...
            pmSwitNostroDao.runReject(batchNo);
            fixQXtract = new FixQXtract();
    ...
    private void readExcel(String param5, String configFile, PmSwiftNostroDao pmSwitNostroDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, pmSwitNostroDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, pmSwitNostroDao);
        }
    }

    private void readXLS(String param5, String configFile, PmSwiftNostroDao pmSwitNostroDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpPmSwiftNostro pmSwitNostro;
        TmpPmSwiftNostroPK pmSwitNostroPK;
        String[] propertyNames = {"periodOfTime", "ccy", "priority", "nostro", "description","chargeBearer",
        "maxNot","fccProductCode","flgExclude","cmd"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            pmSwitNostroPK = new TmpPmSwiftNostroPK();
            pmSwitNostroPK.setBatchiId(context.get(MapKey.batchNo).toString());
            pmSwitNostroPK.setRecordId(recordId);
            pmSwitNostro = new TmpPmSwiftNostro();
            pmSwitNostro.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            pmSwitNostro.setCompositeId(pmSwitNostroPK);
            try {
                if (xr.nextRow(pmSwitNostro, propertyNames)) {
                    pmSwitNostroDao.insert(pmSwitNostro);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                pmSwitNostro.setStatusReason(ex.toString());
                pmSwitNostro.setFlagStatus(StatusDefinition.REJECTED);
                pmSwitNostroDao.insert(pmSwitNostro);
            }
        }
    }

    private void readXLSX(String param5, String configFile, PmSwiftNostroDao pmSwitNostroDao) {
