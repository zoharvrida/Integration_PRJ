...
import bdsm.service.SknNgReturDebitService;
...
public class SknNgReturInwardDebitAction extends BaseHostAction implements org.apache.struts2.interceptor.ApplicationAware {
	...
	public String doUpdate() {
		...
		SknNgReturInDebitDetail detail = (SknNgReturInDebitDetail) ((Object[]) mapDetail.get(new SknNgPK(this.batchNo, this.recordId)))[1];
		...
		while (it.hasNext()) {
			...
			try {
				...
				this.getLogger().warn("No property of Object \"" + SknNgReturInDebitDetail.class + "\" with name: \"" + key + "\"");
			}
		}
    ...
    public String listFilename() {
		...
		this.listAuditBatch = batchDAO.listByBatchNoTypeAndStatus(
				this.branchNo + "-%",
				Integer.valueOf(SknNgReturDebitService.CODE_DKE), 
				status, 
				new java.util.Date()
			);
	...
	public String listHeader() {
		...
		this.listHeader = headerDAO.listReturByBatchNoAndStatus(this.batchNo, status);
			...
			for (int i=0; i<this.listHeader.size(); i++) {
				...
				for (int j=0; j<countReject.size(); j++) {
					...
					if ((pk.getBatchNo().equals(arr2[0])) && (pk.getRecordId().equals(arr2[1]))) {
						...
						arr3[1] = arr1[1]; // SknNgInOutDebitHeader
	...
	public String listDetail() {
		...
		if (this.batchNo.equals(this.sessionMap.get("batchNo")) == false) {
			...
			SknNgWSAuditTrailDetailDAO detailDAO = new SknNgWSAuditTrailDetailDAO(this.getHSession());
			this.listDetail = detailDAO.listReturByBatchNoAndStatus(this.batchNo, Integer.valueOf(this.recordId), status); 
			/* [0] = SknNgWSAuditTrailDetail; [1] = SknNgReturInDebitDetail; [2] = SPK Error Description */
			...
			for (Object o : this.listDetail) {
				...
				SknNgReturInDebitDetail dOri = (SknNgReturInDebitDetail) ((Object[]) o)[1];
	...
	public String sendToSPKWebService() {
		SknNgReturInDebitHeaderDAO headerDAO = new SknNgReturInDebitHeaderDAO(this.getHSession());
		SknNgReturInDebitDetailDAO detailDAO = new SknNgReturInDebitDetailDAO(this.getHSession());
		SknNgReturInDebitFooterDAO footerDAO = new SknNgReturInDebitFooterDAO(this.getHSession());
		...
		try {
			...
			SknNgReturInDebitHeader modelHeader = headerDAO.get(new SknNgPK(this.batchNo, this.recordId));
			...
			Iterator<SknNgPK> it = mapDetail.keySet().iterator();
			while (it.hasNext()) {
				SknNgReturInDebitDetail modelDetail, modelDetailPersist;
				
				SknNgPK pk = it.next();
				modelDetail = (SknNgReturInDebitDetail) ((Object[]) mapDetail.get(pk))[1];
				listRecordId.add(modelDetail.getCompositeId().getRecordId());
				
				modelDetailPersist = (SknNgReturInDebitDetail) detailDAO.merge(modelDetail);
				...
				sb.append(service.formatFromReturInDetail(modelDetailPersist));
			...
			SknNgReturInDebitFooter modelFooter = footerDAO.getByParentRecordId(this.batchNo, this.recordId);
			footerDAO.evictObjectFromPersistenceContext(modelFooter);
			
			listRecordId.add(modelFooter.getCompositeId().getRecordId());
			sb.append(service.formatFromReturInFooter(modelFooter, CRC16.CRC16(sb.toString(), 0)));

			
			sb.insert(0, service.formatFromReturInHeader(modelHeader) + SknNgService.newLine());
