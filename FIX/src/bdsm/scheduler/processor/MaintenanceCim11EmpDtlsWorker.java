...
import bdsm.scheduler.dao.TmpMaintenanceCim11EmpDtlsDao;
...
import bdsm.scheduler.model.TmpMaintenanceCim11EmpDtlsPK;
 ...
 * @author v00020800
 */
public class MaintenanceCim11EmpDtlsWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process maintenance cif cim11  has been processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process to maintenance cif cim11 has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public MaintenanceCim11EmpDtlsWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_cim11EmpDtls.properties";
        ...
        TmpMaintenanceCim11EmpDtlsDao cim11EmpDtlsDao = new TmpMaintenanceCim11EmpDtlsDao(session);
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            this.readExcel(param5, configFile, cim11EmpDtlsDao); //Must Change
            this.session.flush();
            this.getLogger().info("Import Excel file from Requestor Success");
            cim11EmpDtlsDao.runValidate(batchNo);
            ...
            cim11EmpDtlsDao.runProcess(batchNo);
            ...
            cim11EmpDtlsDao.runReject(batchNo);
            fixQXtract = new FixQXtract();
    ...
    private void readExcel(String param5, String configFile, TmpMaintenanceCim11EmpDtlsDao cim11EmpDtlsDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException, ParseException {
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, configFile, cim11EmpDtlsDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, configFile, cim11EmpDtlsDao);
        }
    }

    private void readXLS(String param5, String configFile, TmpMaintenanceCim11EmpDtlsDao cim11EmpDtlsDao) throws FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        
        XLSReader xr = XLSReader.getInstance(param5, configFile);
        TmpMaintenanceCim11EmpDtls cim11EmpDtls;
        TmpMaintenanceCim11EmpDtlsPK cim11EmpDtlsPK;
        String[] propertyNames = {"compositeId.cif", "occupation", "tenure", "designation", "name", "address1", "address2",
            "address3", "city", "state", "country", "postalCode", "mobile", "emailID", "phone", "telexNo", "faxNo", "noOfYears", "retirementAge"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            cim11EmpDtlsPK = new TmpMaintenanceCim11EmpDtlsPK();
            cim11EmpDtlsPK.setBatchiId(context.get(MapKey.batchNo).toString());
            cim11EmpDtlsPK.setRecordId(recordId);
            cim11EmpDtls = new TmpMaintenanceCim11EmpDtls();
            cim11EmpDtls.setFlagStatus(StatusDefinition.UNAUTHORIZED);
            cim11EmpDtls.setCompositeId(cim11EmpDtlsPK);
            try {
                if (xr.nextRow(cim11EmpDtls, propertyNames)) {
                cim11EmpDtlsDao.insert(cim11EmpDtls);
                }
            } catch (Exception ex) {
                this.getLogger().error(ex, ex);
                cim11EmpDtls.setStatusReason(ex.toString());
                cim11EmpDtls.setFlagStatus(StatusDefinition.REJECTED);
                cim11EmpDtlsDao.insert(cim11EmpDtls);
            }
        }
    }

    private void readXLSX(String param5, String configFile, TmpMaintenanceCim11EmpDtlsDao cim11EmpDtlsDao) {
        this.getLogger().info("START XLSXReader : ");
        XLSXReader xr = XLSXReader.getInstance(param5, configFile);
        this.getLogger().info("END XLSXReader : " + param5 + " " + configFile);


        TmpMaintenanceCim11EmpDtls cim11EmpDtls;
        TmpMaintenanceCim11EmpDtlsPK cim11EmpDtlsPK;
        String[] propertyNames = {"compositeId.cif", "occupation", "tenure", "designation", "name", "address1", "address2",
            "address3", "city", "state", "country", "postalCode", "mobile", "emailID", "phone", "telexNo", "faxNo",
            "noOfYears", "retirementAge"};
        Integer recordId = 0;
        while (xr.hasNextRow()) {
            recordId += 1;
            cim11EmpDtlsPK = new TmpMaintenanceCim11EmpDtlsPK();
