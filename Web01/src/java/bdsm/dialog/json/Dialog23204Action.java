...
public class Dialog23204Action extends BaseDialogAction {
	private static final String SKN_NG_RETRIEVE_INCOMING_CREDIT = "SknNgIncomingCredit_list.action";
	private static final String NAMMENU = "23204";
	...
	public List doList() throws Exception {
		this.getLogger().info("batchNo : " + this.getIDBATCH());
		this.getLogger().info("Mode : " + this.getMode());
		...
		if (StringUtils.isBlank(this.getMode())) {
			...
			batchNos.append(BdsmUtil.leftPad(this.session.get(Constant.C_CODEBRANCH).toString(), 5, '0')).append(stDate).append(this.getIDBATCH());
	...
	/**
	 * @return the identitasPesertaPengirim
	 */
	...
	/**
	 * @param identitasPesertaPengirim the identitasPesertaPengirim to set
	 */
