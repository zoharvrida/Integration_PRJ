 ...
 * @author NCBS
 */
public class FcrLtaWorker extends BaseProcessor {
    ...
    private String emailDone = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Process Update BA SITE has been Processed. <br/>"
    ...
    private String emailRejected = "Dear Sir/Madam,<br/>"
            + "<br/>"
            + "Your requested process to update BA SITE has been Rejected by Supervisor. <br/>"
            + "<br/>"
            + "Thanks & regards,<br/>"
            + "- BDSM -";
     ...
     * @param context
     */
    public FcrLtaWorker(Map context) {
        super(context);
    }
    ...
    protected boolean doExecute() throws Exception {
        String configFile;
        configFile = "excelutil_fcrlta.properties";
        UfcrLtaTmpSrcDao ufcrLtaTmpSrcDao = new UfcrLtaTmpSrcDao(session);
        ...
        UfcrLtaTmpSrc UfcrLtaTmpSrc = null;
        ...
        if (status.equals(StatusDefinition.UNAUTHORIZED)) {
            ...
            String outFileName = FileUtil.getDateTimeFormatedFileName(FilenameUtils.getBaseName(context.get(MapKey.param1).toString().replace("BASITE ", "")) + "{hhmmss}.xls");
            getLogger().info("Out File Name : " + outFileName);
            String param5 = context.get(MapKey.param5).toString();
            getLogger().info("Param 5 : " + param5);

            readExcel(param5, configFile, UfcrLtaTmpSrc, ufcrLtaTmpSrcDao);
            ufcrLtaTmpSrcDao.get(BatchID);
            getLogger().info("Remove Bug");
            ufcrLtaTmpSrcDao.runValidateFcrLta(BatchID);
            getLogger().info("Run validate BASITE");
            ...
            if (!sourceProcess.equalsIgnoreCase("sftp")) {
                ...
                getLogger().info("FCRLTAWORKER Register FixQXtract Done");
            }
            return true;
        } else if (status.equals(StatusDefinition.AUTHORIZED)) {
            getLogger().info("Status : AUTHORIZED");
            ufcrLtaTmpSrcDao.runUpdateFcrLta(BatchID);
            getLogger().info("Run update to FCR Done");

            String out2FileName = FileUtil.getDateTimeFormatedFileName(FilenameUtils.getBaseName(context.get(MapKey.param1).toString().replace("RE: BASITE ", "")) + "{hhmmss}.xls");
            getLogger().info("FCRLTAWORKER Register FixQXtract");
            ...
            getLogger().info("FCRLTAWORKER Register FixQXtract");
            ufcrLtaTmpSrcDao.runRejectFcrLta(BatchID);
            fixQXtract = new FixQXtract();
    ...
    private void readExcel(String param5, String configFile, UfcrLtaTmpSrc ufcrLtaTmpSrc, UfcrLtaTmpSrcDao ufcrLtaTmpSrcDao) throws FIXException, FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        loadConfig(configFile);
        if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xls")) {
            readXLS(param5, ufcrLtaTmpSrc, ufcrLtaTmpSrcDao);
        } else if (FilenameUtils.getExtension(param5).equalsIgnoreCase("xlsx")) {
            readXLSX(param5, ufcrLtaTmpSrc, ufcrLtaTmpSrcDao);
        }
    }

    private void readXLS(String param5, UfcrLtaTmpSrc ufcrLtaTmpSrc, UfcrLtaTmpSrcDao ufcrLtaTmpSrcDao) throws FIXException, FileNotFoundException, IOException {
        ...
        try {
            ...
            for (int i = rowDataStartOn - headerRow; i < rowCnt - headerRow; i++) {
                ...
                ufcrLtaTmpSrc = new UfcrLtaTmpSrc();
                ufcrLtaTmpSrc.setMacroName(getStringCellVal(row.getCell(j)));
                ufcrLtaTmpSrc.setMacroValue(getStringCellVal(row.getCell(j + 1)));
                ufcrLtaTmpSrc.setNobatch(context.get(MapKey.batchNo).toString());
                ufcrLtaTmpSrcDao.insert(ufcrLtaTmpSrc);
            }
        } finally {
            file.close();
        }
    ...
    private void readXLSX(String param5, UfcrLtaTmpSrc ufcrLtaTmpSrc, UfcrLtaTmpSrcDao ufcrLtaTmpSrcDao) throws FIXException, FileNotFoundException, IOException, InvalidFormatException, OpenXML4JException, SAXException, ParserConfigurationException {
        ...
        processSheet(styles, strings, stream, ufcrLtaTmpSrc, ufcrLtaTmpSrcDao);
        stream.close();
    }
    ...
    public void processSheet(
            ...
            InputStream sheetInputStream, UfcrLtaTmpSrc ufcrLtaTmpSrc, UfcrLtaTmpSrcDao ufcrLtaTmpSrcDao)
        ...
        ContentHandler handler = new MyXSSFSheetHandler(styles, strings, minColumns, System.out, ufcrLtaTmpSrc, ufcrLtaTmpSrcDao);
    ...
    private void loadConfig(String confFile) throws FileNotFoundException, IOException {
        ...
        InputStream in = FcrLtaWorker.class.getClassLoader().getResourceAsStream(confFile);
        properties.load(in);
    ...
    class MyXSSFSheetHandler extends DefaultHandler {
        ...
        private UfcrLtaTmpSrc ufcr;
        private UfcrLtaTmpSrcDao ufcrDao;
        ...
        public MyXSSFSheetHandler(
                ...
                ReadOnlySharedStringsTable strings,
                int cols,
                PrintStream target, UfcrLtaTmpSrc ufcrLtaTmpSrc, UfcrLtaTmpSrcDao ufcrLtaTmpSrcDao) {
            ...
            this.ufcr = ufcrLtaTmpSrc;
            this.ufcrDao = ufcrLtaTmpSrcDao;
            this.ufcr = new UfcrLtaTmpSrc();
        }
        ...
        public void endElement(String uri, String localName, String name)
            ...
            if ("v".equals(name)) {
                ...
                if (thisColumn == 1) {
                    ufcr.setMacroName(thisStr);
                }
                if (thisColumn == 2) {
                    ufcr.setMacroValue(thisStr);
                }
